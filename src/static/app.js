          ...
          <div class="participants-section">
            ...
            ${details.participants.length > 0
              ? `<ul class="participants-list">${details.participants
                  .map((participant) => `<li class="participant-item">${participant} <button class="participant-remove" data-activity="${encodeURIComponent(
                    name
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
          // Refresh activities and select so UI shows the new participant immediately
          fetchActivities();
      } else {
